...
| D6 | Own top-level group `yasrt/` for the code (`yasrt/cli`); the **image** in `devops/images/yasrt`; component templates in the existing `devops/ci-cd-components/release-tools` | Module path `git.ole-hartwig.eu/yasrt/cli`. A product-scoped top-level group follows the pattern already set by `cura-collect-ai`, `nozzleops`, `ai-ready-platform` and `pinup`. The image belongs under `devops/images` because that is where the build pipelines and the signing variables live — a new top-level group inherits none of `devops/`'s `COSIGN_*`, which is a concrete cost of the split. The image repository fetches the released binary from `yasrt/cli`'s package registry with a job token and verifies it against `SHA256SUMS`, the same division `deploy-opkssh` uses. The component catalogue stays where consumers already look, so migrating a repository remains a one-word change |
