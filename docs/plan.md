...
| D6 | Go code in a new repo under `devops/`; component as a template inside `release-tools` | Module path `git.ole-hartwig.eu/devops/yasrt`. Keeps the component catalogue in one place and makes parallel operation a one-word change for consumers |
